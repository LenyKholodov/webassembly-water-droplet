...
# Build profile. Default keeps the wasm source map for in-browser debugging.
# `make RELEASE=1` builds a lean binary: no source map (drops the ~700 KB .map and the
# in-wasm debug info) + a link-time wasm-opt -O3 pass. Switch profiles with a clean build:
#   make clean && make RELEASE=1
# (-flto is intentionally not used: it conflicts with emscripten's exception handling here -
...
clean:
	@echo Cleaning...
	@rm -rf $(TMP_DIR) $(TARGET) $(OUT_DIR)/index.wasm $(OUT_DIR)/index.wasm.map
